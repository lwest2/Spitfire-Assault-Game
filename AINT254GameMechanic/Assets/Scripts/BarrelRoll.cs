...
public class BarrelRoll : MonoBehaviour
{
    ...
    void Update()
    {
        m_barrelRollInput = Input.GetButtonDown("right bumper");    // input for right barrel roll
    ...
    private void FixedUpdate()
    {
        ...
        // if left bumper...
        else if (m_left && m_barrelRollActive)
        {
            if (m_rotation < m_predictUp.z + 180)
            {
                m_rotateAmount = Mathf.Clamp01(m_initialJumpSpeed + ((m_rotation / 180) * m_rotateAcceleration));
        ...
        if (m_barrelRollActive && m_right)
        {
            ...
            transform.Rotate(Vector3.forward * -m_curRotate);
        }
